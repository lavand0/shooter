...
a = resource_path(os.path.join('data/audio', 'jump.wav'))
jump_fx = pygame.mixer.Sound(a)
...
class Soldier(pygame.sprite.Sprite):
    def __init__(self, char_type, x, y, scale, speed, ammo, grenades):
        ...
        for animation in animation_types:
            ...
            # specific way to files
            way = resource_path(f'data/img/{self.char_type}/{animation}')
            num_of_frames = len(os.listdir(way))
            for i in range(num_of_frames):
                # specific way to files
                way = resource_path(os.path.join(f'data/img/{self.char_type}/{animation}', f'{i}.png'))
                img = pygame.image.load(way).convert_alpha()
    ...
    def move(self, moving_left, moving_right):
        ...
        if self.char_type == 'player':
            if (self.rect.right > SCREEN_WIDTH - SCROLL_THRESH and
                bg_scroll < (world.level_lenght * TILE_SIZE) - SCREEN_WIDTH) or \
    ...
    def shoot(self):
        if self.shoot_cooldown == 0 and self.ammo > 0:
            ...
            bullet = Bullet(self.rect.centerx + int(self.rect.size[0] * 0.75 * self.direction),
            ...
            for x, tile in enumerate(row):
                if tile >= 0:
                    ...
                    if tile >= 0 and tile <= 8:
                        ...
                        health_bar = HealthBar(10, 10, player.health, player.max_heath)

                    elif tile == 16:  # create enemies
                        # if I want to have the heath bar to be above the enemies
                        # then maybe I need to make it a part of a 'Soldier' class
    ...
    def update(self):
        ...
        if pygame.sprite.collide_rect(self, player):
            ...
            if self.item_type == 'Health':
                player.health += 25
                if player.health > player.max_heath:
                    player.health = player.max_heath
...
class HealthBar():
    def __init__(self, x, y, heath, max_health):
        self.x = x
        self.y = y
        self.health = heath
        self.max_heath = max_health

    def draw(self, health):
        # update with new health
        self.health = health
        # calculate heath ratio
        ratio = self.health / self.max_heath
...
player = Soldier('player', 100, 400, 1.65, 5, 20, 5)
health_bar = HealthBar(10, 10, player.health, player.max_heath)
# if I want to have the heath bar to be above the enemies
# then maybe I need to make it a part of a 'Soldier' class

enemy = Soldier('enemy', 400, 300 - 54, 1.65, 2, 20, 0)
enemy_group.add(enemy)

# create empty tile list
world_data = [[-1] * COLS for _ in range(ROWS)]
# load in level data and create the world
# specific path to files
way = resource_path(os.path.join(f'data', f'level{level}_data.csv'))
...
while run:
    ...
    if start_game == False:
        ...
        for x in range(player.grenades):
            screen.blit(grenade_img, (158 + (x * 15), 62))
